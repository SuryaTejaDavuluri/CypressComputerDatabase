...
describe('Add New Computer', function () {
    ...
    it.skip('Add New Computer with all available fields', function () {

        cy.AddComputer('TEST', '2005-02-02', '2008-07-07', 'Sony', 'Create')
    })

    it.skip('Add New Computer with only Computer Name', function () {
        cy.AddComputer('TEST2', '', '', '', 'Create')
    })
    ...
    it.skip('Add New Computer and Cancel before creation', function () {
    ...
    //     cy.AddComputer(this.data.Computer[0].Name, this.data.Computer[0].IntroducedDate, this.data.Computer[0].DiscontinuedDate, this.data.Computer[0].Company)
    // })

    // it('Verify New Computer creation with Excel data', function () {
    //     cy.fixture('xlsxData').then(function (xc) {
    //         for (let i = 0; i < rowsLength; i++) {
    //             cy.AddComputer(xc.rows[i].ComputerName, xc.rows[i].IntroducedDate, xc.rows[i].DiscontinuedDate, xc.rows[i].Company)
    //         }
    //     })
    // })
